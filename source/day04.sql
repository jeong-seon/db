...
/*
    ...
    ==> Relation DataBase Management System 에서는 데이터의 중복을 피하기 위해서
        ...
        그런데 분리된 테이블에서 데이터를 추출해낼 때 사용하는 문법이 JOIN 이다.
        
        참고 ]
            오라클 역시 ER 형태의 데이터베이스이다.
            ER
                엔티티(테이블)끼리의 관계를 이야기하고
                이런 관계들로 테이블을 관리하는 데이터베이스를 관계형데이터베이스 라고 한다.
...
/*
    ...
    2. Inner Join
        ...
        참고 ]
            INNER JOIN 이 가장 일반적인 조인이기 때문에 INNER 이라는 단어를 생략하면
            INNER JOIN 을 해석해서 실행하게 된다.
    ...
    3. Outer Join
        ...
        형식 ]
            ...
            FROM
                ...
                ==> 이 때 방향은 데이터가 있는 테이블 방향을 기술하면 된다.
            ON
                조인조건...
            ;
...
SELECT
    ename 사원이름, job 직급, TO_CHAR(hiredate, 'yy"년도"') 입사년도, dname 부서이름
FROM
    emp e INNER JOIN dept d
ON -- 조인조건
    e.deptno = d.deptno
...
/*
    ...
    ==> 질의명령 안에 다시 질의명령을 포함하는 경우
        포함되는 그 질의명령을 서브질의 또는 서브쿼리 라고 부른다.
        ...
        예 ]
            ...
            이 때 위의 질의명령을 아래 메인질의명령에서 조회해서 사용해야 한다.
